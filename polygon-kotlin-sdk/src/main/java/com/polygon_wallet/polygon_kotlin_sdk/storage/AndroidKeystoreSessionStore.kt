...
internal class AndroidKeystoreSessionStore(
    ...
    override suspend fun <T> withPrivateKey(block: suspend (ByteArray) -> T): T {
        val persisted = requireNotNull(readPersistedEnvelope()) { "No persisted Sequence wallet session" }
    ...
    companion object {
        ...
        private const val DEFAULT_KEY_ALIAS = "sequence-wallet-session-key"
        private const val DEFAULT_FILE_NAME = "sequence-wallet-session.json"
    }
}
