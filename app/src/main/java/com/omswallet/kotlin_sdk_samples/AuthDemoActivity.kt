...
import com.omswallet.kotlin_sdk.generated.waas.WebRpcError
...
class AuthDemoActivity : AppCompatActivity() {
    ...
    private fun bindActions() {
        ...
        findViewById<MaterialButton>(R.id.startEmailSignInButton).setOnClickListener {
            launchAction(
                ...
                val response = sdk.signInWithEmail(email)
                authStatusView.text = buildString {
                    append("Code requested for ")
                    append(response.loginHint ?: email)
                }
                signerAddressView.text = "Signer address: ${sdk.signerAddress ?: "none"}"
                showPendingCodeStep()
    ...
    private fun renderSessionState() {
        if (sdk.signerAddress == null && sdk.walletAddress == null) {
            resetUiForNoSession()
            return
        }
        ...
        walletAddressView.text = "Wallet address: ${sdk.walletAddress}"
        authCard.visibility = View.GONE
        codeStepContainer.visibility = View.GONE
        walletActionsContainer.visibility = View.VISIBLE
    }
