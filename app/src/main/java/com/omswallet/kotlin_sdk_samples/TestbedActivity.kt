...
class TestbedActivity : AppCompatActivity() {
    ...
    private fun bindActions() {
        ...
        findViewById<MaterialButton>(R.id.signInButton).setOnClickListener {
            launchAction("Start email sign-in") { sdk ->
                val response = sdk.signInWithEmail(requireText(emailInput, "Email"))
                appendLog(
        ...
        findViewById<MaterialButton>(R.id.confirmCodeButton).setOnClickListener {
            launchAction("Confirm email code and resolve wallet") { sdk ->
                val wallet = sdk.completeEmailSignIn(
        ...
        findViewById<MaterialButton>(R.id.signMessageButton).setOnClickListener {
            launchAction("Sign message") { sdk ->
                ...
                val result = sdk.signMessage(chainId = chainId, message = message)
        ...
        findViewById<MaterialButton>(R.id.verifySignatureButton).setOnClickListener {
            launchAction("Verify last signature") { sdk ->
                val result = sdk.utils.verifySignature(
                    ...
                    walletAddress = requireNotNull(sdk.walletAddress) { "No wallet selected" },
    ...
    private fun launchAction(label: String, action: suspend (OmsWallet) -> Unit) {
        uiScope.launch {
            appendLog(">> $label")
            runCatching {
                action(requireSdk())
    ...
    private fun requireSdk(): OmsWallet {
    ...
    private fun currentEnvironment(): OmsWalletEnvironment =
    ...
    private fun renderSession() {
        val sdk = runtime?.sdk
        if (sdk == null || (sdk.signerAddress == null && sdk.walletAddress == null)) {
            currentSignerView.text = "Signer: none"
            currentWalletView.text = "Wallet: none"
            return
        }

        currentSignerView.text = "Signer: ${sdk.signerAddress ?: "none"}"
        currentWalletView.text = "Wallet: ${sdk.walletAddress ?: "pending selection"}"
    }
