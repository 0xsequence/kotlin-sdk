...
internal class AndroidKeystoreSessionStore(
    ...
    override fun save(snapshot: OmsWalletSessionSnapshot, privateKey: ByteArray?) {
        ...
        val encryptedPrivateKey = when {
            ...
            else -> throw IllegalStateException("Cannot persist OMS Wallet session metadata without a private key")
        }
    ...
    override suspend fun <T> withPrivateKey(block: suspend (ByteArray) -> T): T {
        val persisted = requireNotNull(readPersistedEnvelope()) { "No persisted OMS Wallet session" }
    ...
    companion object {
        ...
        private const val DEFAULT_KEY_ALIAS = "oms-wallet-session-key"
