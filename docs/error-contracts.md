...
| `client.wallet.isValidMessageSignature`, `isValidSolanaMessageSignature`, `isValidTypedDataSignature` | WaaS validation backend failure | `OMSWalletRequestException` or `OMSWalletResponseException` with validation operation | Retry based on SDK code/status; log upstream detail | Present | `PublicErrorContractsTest` |
